...
The temporary POS-004 shift policy automatically opens one development shift per device with zero opening cash when the sale screen is entered. POS-005 will replace this convenience with explicit opening, reconciliation, and closing. Receipt sequences are scoped by device and UTC date inside SQLite; UTC avoids an unavailable timezone conversion dependency in the offline critical path.
