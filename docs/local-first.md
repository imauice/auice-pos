...
POS-006 keeps StockMovement as the inventory source of truth. Manual receiving, opening, adjustment, and waste operations validate the latest local catalog, append one movement, and insert its stable outbox event in the same SQLite transaction. Concurrent movements can safely append independently; a balance query after commit includes the new movement. Negative stock is preserved for offline and operational reconciliation.
