...
import { CURRENCY, DEVICE_PLATFORMS, PAYMENT_METHODS, SALE_STATUSES, SHIFT_STATUSES, STOCK_MOVEMENT_TYPES, UNIT_CATEGORIES, UUID_PATTERN } from '../domain.constants';
...
const integer = (min?: number) => ({ type: Number, required: true, min, validate: Number.isInteger });
...
export const SaleItemSchema = new Schema({ id: { type: String, required: true, match: UUID_PATTERN }, saleId: { type: String, required: true, match: UUID_PATTERN }, productId: { type: String, required: true, match: UUID_PATTERN }, productUnitId: { type: String, required: true, match: UUID_PATTERN }, productNameSnapshot: { type: String, required: true }, skuSnapshot: String, unitCodeSnapshot: { type: String, required: true }, unitNameSnapshot: { type: String, required: true }, barcodeSnapshot: String, quantityMinor: integer(1), quantityScale: integer(1), conversionNumeratorSnapshot: integer(1), conversionDenominatorSnapshot: integer(1), baseQuantityMinor: integer(1), baseQuantityScale: integer(1), unitPriceMinor: money, subtotalMinor: money, discountMinor: money, taxMinor: money, totalMinor: money, createdAt: { type: Date, required: true } }, { _id: false, versionKey: false });
export const PaymentSchema = new Schema({ id: { type: String, required: true, match: UUID_PATTERN }, saleId: { type: String, required: true, match: UUID_PATTERN }, branchId: { type: String, required: true, match: UUID_PATTERN }, deviceId: { type: String, required: true, match: UUID_PATTERN }, method: { type: String, enum: PAYMENT_METHODS, required: true }, amountMinor: money, currency: { type: String, enum: CURRENCY, required: true }, reference: String, paidAt: { type: Date, required: true }, createdAt: { type: Date, required: true } }, { _id: false, versionKey: false });
export const SaleSchema = new Schema({ ...syncableFields, deviceId: { type: String, required: true, match: UUID_PATTERN }, shiftId: { type: String, required: true, match: UUID_PATTERN }, receiptNumber: { type: String, required: true }, status: { type: String, enum: SALE_STATUSES, required: true }, currency: { type: String, enum: CURRENCY, required: true }, subtotalMinor: money, discountMinor: money, taxMinor: money, totalMinor: money, paidMinor: money, changeMinor: money, itemCount: integer(0), soldAt: { type: Date, required: true }, voidedAt: Date, voidReason: String, items: { type: [SaleItemSchema], required: true }, payments: { type: [PaymentSchema], required: true } }, options);
SaleSchema.index({ branchId: 1, receiptNumber: 1 }, { unique: true });

export const StockMovementSchema = new Schema({ id: { type: String, required: true, unique: true, match: UUID_PATTERN }, branchId: { type: String, required: true, match: UUID_PATTERN }, deviceId: { type: String, required: true, match: UUID_PATTERN }, productId: { type: String, required: true, match: UUID_PATTERN }, type: { type: String, enum: STOCK_MOVEMENT_TYPES, required: true }, sourceUnitId: { type: String, required: true, match: UUID_PATTERN }, sourceUnitCodeSnapshot: { type: String, required: true }, sourceUnitNameSnapshot: { type: String, required: true }, sourceQuantityMinor: integer(1), sourceQuantityScale: integer(1), conversionNumeratorSnapshot: integer(1), conversionDenominatorSnapshot: integer(1), baseQuantityMinor: integer(), baseQuantityScale: integer(1), referenceType: { type: String, required: true }, referenceId: { type: String, required: true }, occurredAt: { type: Date, required: true }, note: String, createdAt: { type: Date, required: true }, version: integer(1) }, options);
StockMovementSchema.index({ branchId: 1, productId: 1, occurredAt: 1 });
StockMovementSchema.pre('validate', function (this: { type: string; baseQuantityMinor: number }) { assertMovementDirection(this.type, this.baseQuantityMinor); });
