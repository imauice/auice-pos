...
POS-004 adds offline checkout. The sale screen searches the local catalog by name, SKU, or barcode; bottle and case units remain separate cart lines with captured prices. Checkout accepts cash, commits the sale, snapshot lines, payment, stock movements, and pending outbox events in one SQLite transaction, and makes the receipt and history available immediately. Receipt dates use UTC for deterministic offline sequencing; the branch timezone remains stored for localized display.
